...
const MediumClap = ({ children, onClap }) => {
  ...
  const componentJustMounter = useRef(true);
  useEffect(() => {
    if (!componentJustMounter.current) {
      onClap && onClap(clapState);
    }
    componentJustMounter.current = false;
  }, [count]);
...
const CountTotal = () => {
  ...
  return (
    <span ref={setRef} className={styles.total} data-refkey="countTotalRef">
      {countTotal}
    </span>
  );
};
